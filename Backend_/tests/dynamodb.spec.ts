...
describe('DynamoDB Interface', () => {
    ...
    /* tslint:disable all */
    it('DynamoDB Interface must UpdateItem After being Inserted with Success', async () => {
        ...
        .then(async (result) => sut.queryItemByHashKey({ cartId: mockedCartId }))
        .then((objects) => { objects.length > 0 ? Promise.resolve(true) : Promise.reject('No Data Inserted In Table'); })
        .then(async (result) => sut.updateItem(mockedKeys, { quantity: 20 }))
        .then(async (result) => sut.queryItemByHashKey(mockedKeys))
        .then((objects) => expect(objects[0]['quantity']).to.eqls(20));
